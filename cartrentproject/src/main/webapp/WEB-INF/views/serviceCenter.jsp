			                ...
			                <tbody>					
				                <c:forEach items="${list}" var="list" varStatus="status">
					                <tr>
					                    ...
					                    <td>${list.division}</td>
					                    <td class="text-l pal0" onclick="location.href='${path}/serviceCenter/noticeDetail/${list.no}?number=${number}&moVal=${moVal}&moKind=${moKind}'" style="padding-left:6px !important;">
...
<div class="paging">
		...
		<c:if test="${SSize/listNum > startNum + listNum-1}"> 	<c:set var="sn" value="${startNum+listNum}"/> </c:if>
		<c:if test="${SSize/listNum <= startNum + listNum-1}"> 	<c:set var="sn" value="${pagingSize}"/> </c:if>
